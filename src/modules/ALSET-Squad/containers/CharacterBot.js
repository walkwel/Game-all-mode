...
class Character extends Component {
  ...
  loop = () => {
    if (this.props.botPositions && this.props.botCollectives.length) {
      var direction = this.props.script({
        collectives: this.props.botCollectives[this.props.gameIndex],
        player: this.props.botPositions[this.props.gameIndex],
      });
      //console.log(direction);
      if (!direction) this.props.updateBotDirection({ gameIndex: this.props.gameIndex, direction: 'left' });
      else if (direction.left) this.props.updateBotDirection({ gameIndex: this.props.gameIndex, direction: 'left' });
      else if (direction.right) this.props.updateBotDirection({ gameIndex: this.props.gameIndex, direction: 'right' });
      else if (direction.up) this.props.updateBotDirection({ gameIndex: this.props.gameIndex, direction: 'up' });
      else if (direction.down) this.props.updateBotDirection({ gameIndex: this.props.gameIndex, direction: 'down' });
    }
    ...
    if (this.props.gamesData.playerScore >= gameJsonData.amountToWin)
      ...
      this.props.updateBotSpeed({ gameIndex: this.props.gameIndex, speed: 0 });
    else if (!this.checkBorderCollision()) this.props.updateBotSpeed({ gameIndex: this.props.gameIndex, speed: 0 });
    else
      this.props.updateBotSpeed({
        gameIndex: this.props.gameIndex,
        speed: gameJsonData.games[this.props.gameIndex].character.speed,
      });
  ...
  render() {
    if (this.props.botPositions == null) return <div />;
    switch (this.props.character.type) {
      case 'gnome1':
        return (
          <div id={'bot' + this.props.gameIndex}>
            <Gnome1 characterData={this.props.character} positionData={this.props.botPositions[this.props.gameIndex]} />
          </div>
        );
      case 'blonde':
        return (
          <div id={'bot' + this.props.gameIndex}>
            <Blonde
              ...
              positionData={this.props.botPositions[this.props.gameIndex]}
            />
          </div>
        );
      default:
  ...
  return bindActionCreators(
    {
      ...
      removeBotCollective: removeBotCollective,
      incrementBotScore: incrementBotScore,
