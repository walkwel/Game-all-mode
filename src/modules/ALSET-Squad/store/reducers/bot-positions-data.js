  ...
  switch (action.type) {
    case 'MOVE_BOT': {
      ...
      if (!state[action.payload.gameIndex]) {
        state[action.payload.gameIndex] = {
          x: character.startingPoint.x,
          y: character.startingPoint.y,
        ...
        switch (state[action.payload.gameIndex].direction) {
          case 'up':
            state[action.payload.gameIndex] = {
              ...state[action.payload.gameIndex],
              y: state[action.payload.gameIndex].y - state[action.payload.gameIndex].speed,
            };
            break;
          case 'down':
            state[action.payload.gameIndex] = {
          ...
          case 'left':
            state[action.payload.gameIndex] = {
              ...state[action.payload.gameIndex],
              x: state[action.payload.gameIndex].x - state[action.payload.gameIndex].speed,
            };
            break;
          case 'right':
            state[action.payload.gameIndex] = {
