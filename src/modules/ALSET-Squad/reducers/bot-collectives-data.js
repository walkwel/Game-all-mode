  ...
  switch (action.type) {
    case 'GENERATE_BOT_COLLECTIVES': {
      ...
      var stonesQuant = Math.floor(
        Math.random() * (collectedData.maxAmount - collectedData.minAmount + 1) + collectedData.minAmount,
      );
      for (var i = 0; i < stonesQuant; i++) {
        var stoneObj = { x: 0, y: 0 };
        stoneObj.x =
          Math.floor(Math.random() * (gameConfig.gameWidth / collectedData.size - 0) + 0) * collectedData.size;
        stoneObj.y =
          Math.floor(Math.random() * (gameConfig.gameHeight / collectedData.size - 0) + 0) * collectedData.size;
